...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		Elite::BehaviorTree* pBehaviourTree = new Elite::BehaviorTree(pBlackboard, 
			new BehaviorSelector
			({
				new BehaviorSequence(
				{
					new BehaviorConditional(IsFoodCloseByEnemies),
	...
	Elite::BehaviorTree* pBehaviourTree = new Elite::BehaviorTree(pBlackboard,
		new BehaviorSelector
		({
			//new BehaviorSequence( //i overengineerd this exercise but it was fun :))
			//{
			//	new BehaviorConditional(IsBiggerEnemyClose),
			//	new BehaviorConditional(AvoidEdge),
			//	new BehaviorAction(ChangeToEvadeAndAvoidEdge)
			//}),
			new BehaviorSequence(
			{
				new BehaviorConditional(IsBiggerEnemyClose),
			...
			new BehaviorSequence(
			{
				new BehaviorConditional(IsSmallerEnemyClose),
				new BehaviorAction(ChangeToSeek)
			}),
			new BehaviorSequence(
			{
...
void App_AgarioGame_BT::Update(float deltaTime)
{
	...
	//---------[DEBUG]-------------
	std::cout << typeid(*(m_pUberAgent->GetSteeringBehavior())).name() << '\n';
	//-----------------------------
