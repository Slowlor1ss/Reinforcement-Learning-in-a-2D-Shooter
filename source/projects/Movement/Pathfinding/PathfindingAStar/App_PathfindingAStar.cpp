...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	//Debugging
	if (m_DrawClosedList && !m_cList.empty())
	{
		m_GraphRenderer.HighlightNodes(m_pGridGraph, m_cList, {0.9f,0.2f,0.5f});
	}
	if (m_DrawOpenList && !m_oList.empty())
	{
		m_GraphRenderer.HighlightNodes(m_pGridGraph, m_oList, { 0.5f,0.7f,0.9f });
	}
}
...
#pragma region UI
	//UI
	{
		...
		ImGui::Spacing();
		ImGui::Text("debugging show:");
		ImGui::Checkbox("Closed List", &m_DrawClosedList);
		ImGui::Checkbox("Open List", &m_DrawOpenList);
		ImGui::Spacing();
...
void App_PathfindingAStar::CalculatePath()
{
		...
		&& startPathIdx != endPathIdx)
	{
		...
		m_cList = pathfinder.GetcList();
		m_oList = pathfinder.GetoList();
