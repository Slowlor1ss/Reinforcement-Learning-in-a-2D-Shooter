...
public:
	...
	void UpdateAgentCell(SteeringAgent* const& agent, const Elite::Vector2& oldPos);

	void RegisterNeighbors(SteeringAgent* const& agent, float queryRadius);
	const std::vector<SteeringAgent*>& GetNeighbors() const { return m_Neighbors; }
	int GetNrOfNeighbors() const { return m_NrOfNeighbors; }

	void RenderCells(SteeringAgent* const& agent, float queryRadius)const;
...
private:
	...
	std::vector<int> GetOverlappingCells(const Elite::Vector2& pos, float radius) const;
	Elite::Vector2 GetRowCol(int idx) const;
	Elite::Vector2 GetRowCol(const Elite::Vector2& pos) const;
	int GetIndex(Elite::Vector2 rowCol) const;
};
